...
# ╔═╡ 0a2a6aac-5934-4668-b631-53326d19a7bc
begin
	using Pkg
...
begin
	...
	function mosaic(c::Matrix{Float64};
		...
		rowsum = sum(c,dims=1) # rowsums
    	(nrow,ncol) = size(c) # table dimension
    	n = sum(c) # sample size
    	w = rowsum./n # widths
    	cw = cumsum(vec(w))
    	h = c ./ rowsum # heights
    	...
    	barplot!(ax1,xpos.-repeat(vec(w)/2,inner=nrow),
			vec(h),stack=repeat(1:nrow,ncol),
    		# color=repeat(categorical_colors(:Dark2_8,nrow),ncol),
			color = colors,
			width=repeat(vec(w),inner=nrow),
        	strokewidth=1,gap=0)
    	barplot!(ax2,xpos.-repeat(vec(w)/2,inner=nrow),
			 vec(h),stack=repeat(1:nrow,ncol),
        	 color=colors,
			 width=repeat(vec(w),inner=nrow),
        	 strokewidth=1,gap=0)
    	l = colors .|> (col->PolyElement(color=col))
    	Legend(f[1,2],l,string.(kron(xnames,ynames)))
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "f6eebde583f7f2018f4c77096c7e813e4be8cf74"
